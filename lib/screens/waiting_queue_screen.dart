...
class _WaitingQueueScreenState extends State<WaitingQueueScreen> {
  ...
  Future<void> _loadWaitingQueue() async {
    ...
    try {
      ...
      for (var i = 0; i < records.length; i++) {
        ...
        final isCurrentUser =
            session != null && record.patientId == session.patientId;
        tickets.add(
          WaitingTicket(
            queueNumber: i + 1,
            record: record,
  ...
  Widget build(BuildContext context) {
    ...
    final displayTicket =
        myTicket ?? (_tickets.isNotEmpty ? _tickets.first : null);
    final bool showMissingNotice =
        session != null && myTicket == null && _tickets.isNotEmpty;
  ...
  Widget _buildBody({
    ...
    if (_errorMessage != null) {
      return _ErrorState(message: _errorMessage!, onRetry: _loadWaitingQueue);
    }
            ...
            child: _tickets.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _QueueCard(ticket: ticket, accent: accent);
                    },
                  ),
          ),
        ),
...
class _QueueCard extends StatelessWidget {
  const _QueueCard({required this.ticket, required this.accent});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Icon(Icons.info_outline, color: Color(0xFFBF5B04)),
          const SizedBox(width: 12),
          Expanded(
            child: Text(
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        Text('현재 대기 중인 환자가 없습니다.', style: theme.textTheme.bodyMedium),
        const SizedBox(height: 4),
        Text(
